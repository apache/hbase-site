import{j as e}from"./components-BCHf_v1I.js";let o=`Data comes in many sizes, and saving all of your data in HBase, including binary
...
requests. When these MOB hfiles are created from a flush or MOB compaction they are placed in a
dedicated mob data area under the hbase root directory specific to the namespace, table, mob
logical region, and column family. In general that means a path structured like:`},{heading:"hbase-mob-overview",content:`With default configs, an example table named 'some\\_table' in the
...
When the hfile of MOB values reaches limit, the MOB hfile is committed to the MOB storage area and
a new one is created. The hfile with reference cells will track the complete set of MOB hfiles it
needs in its metadata.`},{heading:"further-limiting-write-amplification",content:`When using the write amplification optimized compaction mode you need to watch for the maximum
time to compact a single region. If it nears an hour you should read through the troubleshooting
section below Adjusting the MOB cleaner's tolerance for new
hfiles. Failure
...
value that is larger than the largest cell present in the column family. E.g. given a table named
'some\\_table' and a column family named 'foo' we can pick one gigabyte as an arbitrary "bigger than
what we store" value:`},{heading:"change-the-mob-size-threshold",content:"Note that if you are still ingesting data you must ensure this threshold is larger than any cell value you might write; MAX\\_INT would be a safe choice."},{heading:"perform-a-major-compaction-on-the-table",content:'Specifically you are performing a "normal" compaction and not a MOB compaction.'},{heading:"monitor-for-the-end-of-the-major-compaction",content:"Since compaction is handled asynchronously you'll need to use the shell to first see the compaction start and then see it end."},{heading:"monitor-for-the-end-of-the-major-compaction",content:'HBase should first say that a "MAJOR" compaction is happening.'},{heading:"monitor-for-the-end-of-the-major-compaction",content:'When the compaction has finished the result should print out "NONE".'},{heading:"monitor-for-the-end-of-the-major-compaction",content:`Run the *mobrefs* utility to ensure there are no MOB cells. Specifically, the tool will launch a
Hadoop MapReduce job that will show a job counter of 0 input records when we've successfully
...
a new one is created. The hfile with reference cells will track the complete set of MOB hfiles it
needs in its metadata.`}),`
`,e.jsx(n,{type:"warn",title:"Be mindful of total time to complete compaction of a region",children:e.jsxs(i.p,{children:[`When using the write amplification optimized compaction mode you need to watch for the maximum
time to compact a single region. If it nears an hour you should read through the troubleshooting
...
`,e.jsx(n,{type:"info",children:e.jsx(i.p,{children:`This won't cause any functional problem, during next compaction such data will be written out to
the MOB hfiles.`})}),`
...
`]})]})}function k(a={}){const{wrapper:i}=a.components||{};return i?e.jsx(i,{...a,children:e.jsx(l,{...a})}):l(a)}function t(a,i){throw new Error("Expected component `"+a+"` to be defined: you likely forgot to import, pass, or provide it.")}export{o as _markdown,k as default,c as extractedReferences,d as frontmatter,p as structuredData,m as toc};
