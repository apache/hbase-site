import{j as e}from"./components-BCHf_v1I.js";import{_ as i}from"./backup-app-components-ChSULF98.js";let c=`
...
<img alt="Tables Composing The Backup Set" src={__img0} />
...
to recover the data later. All commands are run when logged in as HBase superuser.`},{heading:"scenario-safeguarding-application-datasets-on-amazon-s3",content:`A backup set called *green\\_set* is created as an alias for both the transactions table and the customer table. The backup set can
be used for all operations to avoid typing each table name. The backup set name is case-sensitive and should be formed with only
printable characters and without spaces.`},{heading:"scenario-safeguarding-application-datasets-on-amazon-s3",content:`The first backup of green\\_set data must be a full backup. The following command example shows how credentials are passed to Amazon
...
admin converts the calendar timestamp of the recovery point in time to Unix time because backup IDs are uniquely identified with
Unix time. The backup IDs are listed in reverse chronological order, meaning the most recent successful backup appears first.`},{heading:"scenario-safeguarding-application-datasets-on-amazon-s3",content:"The admin notices that the following line in the command output corresponds with the *green\\_set* backup that needs to be restored:"},{heading:"scenario-safeguarding-application-datasets-on-amazon-s3",content:`The admin restores green\\_set invoking the backup ID and the -overwrite option. The -overwrite option truncates all existing data
in the destination and populates the tables with data from the backup dataset. Without this flag, the backup data is appended to the
...
`,e.jsx(t.p,{children:e.jsx(t.img,{alt:"Tables Composing The Backup Set",src:i})}),`
